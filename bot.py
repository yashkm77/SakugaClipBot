...
def clean_animator_name(name: str) -> str:
    # Remove Sakugabooru prefixes like "Art;"
    if ";" in name:
        name = name.split(";")[-1]
...
async def find_closest_animator_tag(query: str):
    ...
    print(f"Search: {query} | Match: {best} | Score: {best_score}")
...
async def clip(
    ...
    search_name = clean_animator_name(animator).lower()
    ...
    display_name = clean_animator_name(tag).title()
    ...
    for post in clips:
        ...
        if file_url:
            ...
            await interaction.channel.send(file_url)
